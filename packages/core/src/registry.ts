/**
 * Descubre y carga las recetas de packages/core/src/icons/ ** /*.icon.ts
...
import type { Category, IconRecipe, LoadedIcon } from './types.ts';

export const ICONS_DIR = paths.icons;

export const CATEGORIES: Record<Category, { name_es: string; order: number }> = {
...
function walk(dir: string): string[] {
  return readdirSync(dir).flatMap((entry) => {
    ...
    return entry.endsWith('.icon.ts') ? [full] : [];
  });
}

/** Comprueba los metadatos. Devuelve un array de errores (vacio = correcto). */
export function checkMetadata(icon: IconRecipe, file: string): string[] {
  const errors: string[] = [];
  const where = basename(file);
  const expected = basename(file, '.icon.ts');
  ...
  if (icon.budget && !icon.budget.reason) {
    errors.push(`${where}: un budget sin reason es una excepcion sin justificar`);
  }
  return errors;
}

/** Carga todas las recetas, ordenadas por categoria y luego por id. */
export async function loadIcons({ strict = true } = {}): Promise<{
  icons: LoadedIcon[];
  errors: string[];
}> {
  ...
  for (const file of files) {
    const mod = (await import(pathToFileURL(file).href)) as { default: IconRecipe };
