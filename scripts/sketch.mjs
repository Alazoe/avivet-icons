 ...
 *
 *   node scripts/sketch.mjs hen
 *   node scripts/sketch.mjs            (toda la biblioteca)
 */
import { writeFileSync, mkdirSync } from 'node:fs';
import { join } from 'node:path';
import tokens, { ROOT } from './tokens.mjs';
import { loadIcons } from './registry.mjs';
import { body } from './emit.mjs';
...
if (!list.length) {
  console.error(`no existe el icono "${wanted}". Disponibles: ${icons.map((i) => i.id).join(', ')}`);
  process.exit(1);
}

const ink = `fill="none" stroke="#101820" stroke-width="${stroke.width}" ` +
...
const grid = () => {
  ...
  let g = '';
  for (let i = 1; i < 8; i++) {
    g += `<path d="M${i * step} 0V${canvas.size}M0 ${i * step}H${canvas.size}" stroke="${GUIDE}" stroke-width="0.25" opacity="0.5"/>`;
  }
  const { min, max } = canvas.safeArea;
  g += `<rect x="${min}" y="${min}" width="${max - min}" height="${max - min}" fill="none" stroke="${GUIDE}" stroke-width="0.4" stroke-dasharray="2 2"/>`;
  const sq = (canvas.size - keyline.square) / 2;
  g += `<rect x="${sq}" y="${sq}" width="${keyline.square}" height="${keyline.square}" fill="none" stroke="${GUIDE}" stroke-width="0.25" opacity="0.6"/>`;
  g += `<circle cx="${canvas.size / 2}" cy="${canvas.size / 2}" r="${keyline.circle / 2}" fill="none" stroke="${GUIDE}" stroke-width="0.25" opacity="0.6"/>`;
  return g;
};
...
const sheet = (icon) => {
  ...
  for (const px of SIZES) {
    strip += `<g transform="translate(${x},${stripY + (128 - px)}) scale(${px / canvas.size})" ${ink}>${g}</g>`;
    strip += `<text x="${x}" y="${stripY + 146}" font-family="ui-monospace,Menlo,monospace" font-size="11" fill="#5A6875">${px}</text>`;
    x += px + 24;
  }
...
for (const icon of list) {
  const file = join(dir, `${icon.id}.svg`);
  writeFileSync(file, sheet(icon));
